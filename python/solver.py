...
class LatticeSolver(object):
    ...
    def solve(self, N_target, with_fock=True):
        ...
        Sigma_HF_init = {bl: np.zeros((bl_size, bl_size)) for bl, bl_size in gf_struct}
    ...
    def update_rho(self):
        ...
        for bl, size in self.gf_struct:
            e, V = np_eigh(self.e_k_MF[bl].data)
            e -= self.mu
        
            self.rho[bl] = np.einsum('kab,kb,kbc->ac', V, fermi(e), V.conj().T)
    ...
    def update_mu(self, N_target):
        ...
        e_min = np.inf
        e_max = -np.inf
        for bl, size in gf_struct:
            energies[bl] = np.eigvalsh(self.e_k_MF[bl].data)
